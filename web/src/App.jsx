...
function generateYAML(agentName, agentVersion, nodes, edges) {
  ...
  const nodeLines = [
    ...llmNodes.map(n => {
      ...
      return `  - name: ${n.data.label}
    type: llm
    system_prompt: |
      ${prompt}${toolList ? `\n    tools:\n${toolList}` : ""}`;
    }),
    ...toolNodes.map(n => `  - name: ${n.data.label}\n    type: tool_executor`),
  ].join("\n");
...
export default function AgentPortCanvas() {
  ...
  const yaml = generateYAML(agentName, agentVersion, nodes, edges);
  try {
  ...
  return (
    <div style={{ width: "100vw", height: "100vh", background: C.bg, display: "flex", flexDirection: "column", fontFamily: "'JetBrains Mono', monospace", color: C.text }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "0 20px", height: 52, borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        ...
        />
        <div style={{ flex: 1 }} />
        <button onClick={buildAgent} style={{
          background: "#3fb95020", border: `1px solid ${C.green}`, color: C.green,
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div style={{ width: 200, borderRight: `1px solid ${C.border}`, background: C.surface, padding: 16, flexShrink: 0, overflowY: "auto" }}>
          ...
          <div style={{ marginTop: 20, padding: "10px", borderRadius: 6, background: `${C.amber}08`, border: `1px solid ${C.border}` }}>
            <div style={{ fontSize: 9, color: C.textMuted, lineHeight: 1.5 }}>
              Click a node type to add it to the canvas. Drag nodes to reposition. Connect by dragging from a handle.
            </div>
          </div>
        </div>
      ...
      {yamlOutput && (
        <div style={{
          position: "fixed", inset: 0, background: "#00000090", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", borderBottom: `1px solid ${C.border}` }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={copyYAML} style={{
                  background: C.amberDim, border: `1px solid ${C.amber}`, color: C.amber,
                  padding: "4px 12px", borderRadius: 4, fontSize: 10, cursor: "pointer", fontFamily: "inherit", fontWeight: 700
                }}>{copied ? "COPIED ✓" : "COPY"}</button>
                <button onClick={() => setYamlOutput(null)} style={{
